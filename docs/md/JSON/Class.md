...
### Common For Classes And Roles
  ...
  * `Bool` **`:implicit`**
    ...
    Set typeobject explicit/implicit mode. Omitting this argument assumes that the typeoject is implicit unless at least one attribute marked explicitly in which case the entire typeobject becomes explicit. See [*implicit.raku*](../../../examples/implicit.raku).
  ...
  * `Bool` **`:lazy`**
    ...
    When the argument is used all attributes of the typeobject use it as their default, unless their own mode is set manually. See [*decl-lazy.raku*](../../../examples/decl-lazy.raku).

  * `Bool` **`:skip-null`**

    If *True* then attributes of this typeobject are not serialized if their value is undefined. When omitted then value of this parameter is obtained from the configuration object. See [*020-json-basics.rakutest*](../../../t/020-json-basics.rakutest), subtest "Undefineds".
