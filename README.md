...
This module is an alternative to the family of [`JSON::Marshal`](rakudoc:JSON::Marshal:auth<zef:jonathanstowe>), [`JSON::Unmarshal`](rakudoc:JSON::Unmarshal:auth<zef:raku-community-modules>), [`JSON::Class`](rakudoc:JSON::Class:auth<zef:jonathanstowe>) modules. It tries to get rid of their weak points and shifts the locus of control from class' outers to its inners. In other words, the class itself is responsible for its de-/serialization in first place. Though it's perhaps, the primary difference, it's far from being the only one.
  ...
  * **IMPORTANT!**
    ...
    Otherwise you are likely to accidentally pull in [`JSON::Class:auth<zef:jonathanstowe>`](rakudoc:JSON::Class:auth<zef:jonathanstowe>).

Also, this module tries to follow the standards of [`LibXML::Class`](rakudoc:LibXML::Class:auth<zef:vrurg>) module by adapting them for differing domain of JSON. First of all, they share the same view on the locus of responsibility. Second, they try to implement declarative semantics in first place.
...
There is a consequence to this rule: implicit JSONification of attributes skips these with `json-`-prefixed names:
  ...
  * From [examples/json-pfx-attr.raku](file:examples/json-pfx-attr.raku)
...
### Common For Classes And Roles
  ...
  * `Bool` **`:implicit`**
    ...
    See [*examples/decl-implicit.raku*](file:examples/decl-implicit.raku).
  ...
  * `Bool` **`:lazy`**
    ...
    When the argument is used all attributes of the typeobject use it as their default, unless their own mode is set manually. See [*examples/decl-lazy.raku*](file:examples/decl-lazy.raku).

  * `Bool` **`:skip-null`**

    If *True* then attributes of this typeobject are not serialized if their value is undefined. When omitted then value of this parameter is obtained from the configuration object. See [*t/020-json-basics.rakutest*](file:t/020-json-basics.rakutest), subtest "Undefineds".
...
There is an exception though: adverbs `:skip`, `:build`, and `:aliases`, used in any combination, but with no other arguments, doesn't affect the status of attribute's declarant.
  ...
  * `Array[Str:D]` **`:alias`** or `B<:aliases>`
    ...
    This adverb has the same meaning as its counterpart in [`AttrX::Mooish`](rakudoc:AttrX::Mooish): it allows an attribute to have more than one name. **Note** though that this adverb doesn't affect marshalling where only one JSON key name is allowed.
  ...
  * `Bool` **`:serializer(...)`**, `Bool` **`:deserializer(...)`**
    ...
    See 'Custom Marshallers' in [`JSON::Class::Details`](rakudoc:JSON::Class::Details).

  * `Bool:D | Str:D | Code:D` **`:build(...)`**

    Lazily build attribute value upon read unless it's been initialized already from other sources like a constructor argument, or a value from JSON structure. Unless you're familiar with [`AttrX::Mooish`](rakudoc:AttrX::Mooish), then think of this adverb as a kind of [`is default`](https://docs.raku.org/type/Attribute#Trait_is_default) trait but with some specifics.
    ...
    See [*examples/lazy-build.raku*](file:examples/lazy-build.raku) and subtest "Lazy Build" in [*t/020-json-basics.rakutest*](file:t/020-json-basics.rakutest).
...
But what if we know that IDs of books and articles are sufficiently different to tell exactly which is is which? For example, book ID could start with *ISBN:*, whereas article ID starts with a date-based *YYYY-MM-DD:* prefix? In this case custom matchers can distinguish one JSON object from another:
  ...
  * From [examples/book-article-seq.raku](file:examples/book-article-seq.raku)
...
A universal matcher for type can also be set using [`JSON::Class::Config`](rakudoc:JSON::Class::Config) `set-helpers` method.
...
=================
  ...
  * [`JSON::Class::Descriptor`](rakudoc:JSON::Class::Descriptor) **`$*JSON-CLASS-DESCRIPTOR`**
...
-----------------
  ...
  * From [examples/decl-implicit.raku](file:examples/decl-implicit.raku)
...
--------------------------------
  ...
  * From [examples/simple-inherit.raku](file:examples/simple-inherit.raku)
...
Notice that except for `C3`, all other classes and roles are not JSONified. Only attributes of `C1` and `R1` are serialized.
  ...
  * From [examples/incorporation.raku](file:examples/incorporation.raku)
...
--------------------
  ...
  * From [examples/decl-lazy.raku](file:examples/decl-lazy.raku)
...
--------------------
  ...
  * From [examples/lazy-build.raku](file:examples/lazy-build.raku)
...
`C1` specify config defaults but they affect the output only when the immediate instance of the class is serialized. Otherwise if a subclass instance is serialized then its defaults are used. This is to preserve the uniformity of serialization.
  ...
  * From [examples/config-defaults.raku](file:examples/config-defaults.raku)
...
Apparently, this example doesn't cover all of `JSON::Class` sequence features.
  ...
  * From [examples/simple-sequence.raku](file:examples/simple-sequence.raku)
...
----------
  ...
  * From [examples/simple-dictionary.raku](file:examples/simple-dictionary.raku)
...
----------------
  ...
  * From [examples/typed-array.raku](file:examples/typed-array.raku)
...
These two subjects are tightly bound to each other, hence single example for both subjects.
  ...
  * From [examples/config-and-type-map.raku](file:examples/config-and-type-map.raku)
...
There are more than we can fit into this example. We can have individual custom marshalling for hash and array values, and hash keys too. We can specify marshallers for sequence elements too.
  ...
  * From [examples/custom-marshallers.raku](file:examples/custom-marshallers.raku)
...
Here we use custom marshallers too, this time for array values. But there is a trick: serializer and deserializer do not modify every third value. To do so they call `json-I-cant` routine giving up the task to `JSON::Class`
  ...
  * From [examples/skipping-marshalling.raku](file:examples/skipping-marshalling.raku)
...
---------------
  ...
  * **`method to-json(:$config, Bool :$raw, Bool :$pretty, Bool :$sorted-keys, Bool :$enums-as-value, UInt :$spacing)`**
    ...
    Serialize an instance of JSON class. Arguments:
      ...
      * [`JSON::Class::Config:D`](rakudoc:JSON::Class::Config:D) **`:$config`**, **`:%config`**
      ...
      * `Bool` **`:$pretty`**, `Bool` **`:$sorted-keys`**, `Bool` **`:$enums-as-value`**, `UInt` **`:$spacing`**
        ...
        Same as [`JSON::Fast`](rakudoc:JSON::Fast:auth<cpan:TIMOTIMO>) `to-json` arguments.
  ...
  * **`proto method from-json(|)`**
    ...
    Deserialize JSON class from either a JSON string, or raw hash or array.
      ...
      * `Bool` **`:$allow-jsonc`**, `Bool` **`:$enums-as-value`**
        ...
        Same as [`JSON::Fast`](rakudoc:JSON::Fast:auth<cpan:TIMOTIMO>) `from-json` arguments.
  ...
  * **`method json-config-context(&code, :$config, *%twiddles)`**
    ...
    Creates a new configuration context by pulling in defaults for the config from various sources. Invokes user `&code` with `$*JSON-CLASS-CONFIG` context variable set with a configuration object. [`JSON::Class::Details`](rakudoc:JSON::Class::Details) has some more information on how configuration context gets built.

SEE ALSO
========

  * [`JSON::Class::Details`](rakudoc:JSON::Class::Details)

  * [`JSON::Class::Config`](rakudoc:JSON::Class::Config)

  * [`JSON::Class::Descriptor`](rakudoc:JSON::Class::Descriptor)

  * [`JSON::Class::Object`](rakudoc:JSON::Class::Object)

  * [`JSON::Class::Dict`](rakudoc:JSON::Class::Dict)

  * [`JSON::Class::Sequence`](rakudoc:JSON::Class::Sequence)

  * [`JSON::Class::Types`](rakudoc:JSON::Class::Types)

  * [`Changes`](rakudoc:..::ChangeLog)
